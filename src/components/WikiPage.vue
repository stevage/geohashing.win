      ...
      async handler() {
        ...
        if (this.pageId) {
          const raw = await fetch(
            ...
            `https://geohashing.val.run/wiki/?action=parse&page=${this.pageId}&format=json`,
          ).then((res) => {
            return res.json()
          })
          if (raw.error) {
          ...
          this.pageContent = raw.parse.text['*']
            .replace(/\/geohashing\/File/g, 'https://geohashing.site/geohashing/File')
            .replace(/\/geohashing\/Image/g, 'https://geohashing.site/geohashing/Image')
            .replace(/([" ])\/images\//g, '$1https://geohashing.site/images/')
